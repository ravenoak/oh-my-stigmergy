...
| `STIGMERGY_FILE_EDITED_DEBOUNCE_MS` | Debounce window for `file.edited` pheromone publishing (default `3000`). Rapid edits to the same path within the window coalesce into a single trailing-edge publish; edits to different paths debounce independently. Set to `0` to publish immediately (pre-debounce behavior). |
| `STIGMERGY_AGENT_TOKEN` | Bearer token sent on every SBP request when the ledger runs in identity mode (`SBP_AUTH_TOKENS_FILE` set — [ADR-0016](../../docs/adr/0016-sbp-ledger-identity-and-kind.md)). Unset by default (open mode). |
