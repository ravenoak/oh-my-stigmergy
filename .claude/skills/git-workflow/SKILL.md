...
name: git-workflow
description: >-
  Completes Git work for oh-my-stigmergy using feature branches, atomic commits, PR-only
  integration to main, squash/rebase merge, branch cleanup, and git fetch --prune. Use when
  branching, committing, pushing, opening or merging pull requests, cleaning up stale branches,
  or finishing a session that has uncommitted repo changes.
allowed-tools: Bash(git *) Bash(gh *)
---

# Git workflow (oh-my-stigmergy)

Apply for **every** session that touches tracked files: finish with **committed** state on a
**feature branch**, or a **clean** tree if intentionally discarding work.
...
- Integration to **`main`**: **pull request only**; merge via **squash** or **rebase**, not merge
  commits ([docs/guides/github-flow.md](docs/guides/github-flow.md)).
- **CI**: required check **`allium-specs / check`** must be green before merge
  ([`.github/workflows/allium-specs.yml`](.github/workflows/allium-specs.yml)).
...
If `-d` refuses (Git thinks branch not merged): use **`git branch -D`** only after verifying the
PR merged and you do not need the branch — otherwise run `gh pr view` first.
...
Delete merged locals (excludes current branch):
...
`--delete-branch` on merge usually removes the feature branch on origin. If one remains:

```bash
git push origin --delete feature/<obsolete>
```

Do **not** bulk-delete **`dependabot/**`** or active collaborator branches without maintainer
intent.

## 7. If push or merge is blocked (auth / permissions)

1. Ensure **all commits** exist on the **feature** branch locally.
2. Tell the user clearly: push failed; give exact **`git push -u origin ...`** and
...
- CI workflow: [`.github/workflows/allium-specs.yml`](.github/workflows/allium-specs.yml)
