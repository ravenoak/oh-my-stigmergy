...
| FR-1.3 | State-dependent fields and transition graphs constrain allowed state changes; violations are treated as critical at maturity. | P2 | partial | [`spec/transitions.json`](../../spec/transitions.json), [`spec/governance.allium`](../../spec/governance.allium) | **Deterministic today:** [`scripts/verify-transitions-sync.sh`](../../scripts/verify-transitions-sync.sh) (mirrors `.allium` → JSON); Python harness [`packages/transitions`](../../packages/transitions/) + CI unittest job. **Other:** promotion to `implemented` requires an Allium AST extractor (no duplicated transition source). |
