...
**Medium-centric multi-agent orchestration** in the OpenCode plugin ([ADR-0013](0013-stigmergic-opencode-orchestration.md)) is **allowed**: coordination state remains **visible on the ledger** (pheromones, claims, intensities) with optional stance→model policy—not a single omniscient router prompt. This does **not** revive hierarchical mega-orchestration as the coordination spine.
...
- OpenCode orchestration tools ([ADR-0013](0013-stigmergic-opencode-orchestration.md)) must remain **SBP-backed** and policy-driven; FR-6.x verification applies.
