...
| Item | Phase | Dependency | Next unblocker |
|------|-------|------------|----------------|
| `packages/orchestrator` — deterministic phase orchestrator over the SBP medium | P3+ | **Shipped:** ledger identity/kind (reduced, FR-9.1), delivery floors (FR-10.x), the WorkOrder payload profile (FR-11.1), and the human-as-orchestrator bridge itself (FR-11.2, `devtools/bridge/`). **Outstanding:** the bridge must be *used* for 1–2 real deliveries before the criteria below can be derived. | **Gated on a pre-registered decision:** [`docs/research/orchestrator-decision-preregistration.md`](research/orchestrator-decision-preregistration.md) — status `SCAFFOLD`, criteria not yet frozen; a Baseline estimation plan describes how thresholds get derived from real bridge usage (phase-management-only: cycle count, measured toil, machine-avoidable phase-management failures) **before** the observation window opens. Promote to an FR row + positioning ADR **only** if the frozen criteria return `Justified` against the log — never by a self-signed ADR alone. A `Rejected-for-window` verdict is a valid, constitution-honoring outcome recorded in synthesis docs, not a failure; it reopens only on a named trigger (second contributor, second adopting repo, sustained toil trend). [ADR-0003](adr/0003-stigmergy-vs-orchestrator.md)'s rejection of a central mega-orchestrator stands unamended regardless of outcome — any future E0 must show the ledger, not a private orchestrator log, stays authoritative for "what's next" (state-locality rule + standing replay/cold-start drill). |
...
| `devtools/allium-cli.version` stuck at 3.5.0 — `allium-parser` 3.5.1+ rejects `module Name { }` block syntax used by 3 `tests/fixtures/crucible/*.allium` fixtures | P1/P4 | [#77](https://github.com/ravenoak/oh-my-stigmergy/issues/77) | Investigate upstream `juxt/allium-tools` 3.5.1–3.5.3 changes; migrate fixtures off block-wrapper syntax (or confirm regression and file upstream) before the CLI pin can move past 3.5.0. |
| `packages/sbp-server/package.json` missing `engines.node` floor (better-sqlite3 13 requires Node >=22) | P3 | [#78](https://github.com/ravenoak/oh-my-stigmergy/issues/78) | Add `"engines": {"node": ">=22"}`; consider `packages/opencode-plugin` too. |
| `actions/setup-node` v7.0.0 bump in `npm-publish.yml` unexercised by regular CI (`workflow_dispatch`-only) | — | [#79](https://github.com/ravenoak/oh-my-stigmergy/issues/79) | Verify at next real `npm publish` dispatch; revert to v6.4.0 pin if OIDC trusted publishing breaks. |
| `gitleaks/gitleaks-action` major v3.0.0 available; repo pinned to `v2` branch SHA | — | [#80](https://github.com/ravenoak/oh-my-stigmergy/issues/80) | Review v3 migration notes; bump with a distillation waiver if compatible. |
