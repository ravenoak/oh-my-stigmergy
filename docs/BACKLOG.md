...
| OpenCode / IDE bridge — MCP or plugin that publishes SBP pheromones and pulls graph/`load_node` context into agent sessions | P3+ | [ADR-0005](adr/0005-conflict-resolution-governance.md) for delegation-shaped features; HTTP/MCP contract | **Not shipped.** Complements [oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent) (orchestrator harness) with this repo’s **environment** (SBP + graph); requires Phase 11-style charter, target FR/NFR rows, RTM verification language, and possibly a new ADR for MCP vs CLI-only integration. |
